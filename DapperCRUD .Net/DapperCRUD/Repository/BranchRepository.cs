...
namespace DapperCRUD.Repository
{
    public class BranchRepository : IBranchRepository
    {
        ...
        public async Task<Branch> GetByIdAsync(Int64 id)
        {
            ...
            using (var connection = _context.CreateConnection())
            {
                var result = await connection.QuerySingleOrDefaultAsync<Branch>(query, new { Id = id });
                return result;
            }
        }
        public async Task Create(Branch _Branch)
        {
            var query = "INSERT INTO " + typeof(Branch).Name + " (Name, Description, CreatedDate,UpdatedDate)" +
